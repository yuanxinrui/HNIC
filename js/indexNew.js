...
app.controller('myCtrl', function($sce,$scope) {
    $scope.addElementSlides = function(){
        $scope.slideHtml =  $sce.trustAsHtml("                <div class=\"swiper-slide\" ng-repeat=\"x in arrUrlTest track by $index\">" +
            "                <div class=\"report_form\">" +
            "                <iframe id=\"{{$index}}\" ng-src=\"{{x.url}}\" width=\"100%\" scrolling=\"no\"></iframe>" +
            "                </div>" +
            "                </div>" );
    };
    //轮播图参数配置
    ...
    $scope.changeFrameHeight = function(){
        ...
        $('iframe').each(function (index) {
            document.getElementById(index).style.height = clinetHeight - 54 +"px";
        });
    }
    ...
    $scope.reportTitleClick = function(index){
        ...
        $scope.arrChild[index].childIdList.forEach(function(obj){
            console.log(obj.id);
            var testId = 'http://10.191.47.199:28081/spreadsheet/vision/openresource.jsp?resid=' + obj.id + '&username=demo&password=demo&showtoolbar=false';
            var reportUrl = $sce.trustAs($sce.RESOURCE_URL,testId);
            $scope.urlTest.push({url:reportUrl});
        });
    };
    $scope.arrUrlTest = [];
    $scope.arrUrlTest = $scope.urlTest;
